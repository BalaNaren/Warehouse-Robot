...
class WarehouseEnv(gym.Env):
    ...
    def step(self, action):
        ...
        reward = 1 if done else -0.1 - 0.01  # Reward for completing the task, small penalty otherwise
...
def train_agent(env, agent, episodes, max_steps_per_episode):
    ...
    for episode in range(episodes):
        ...
        state = env.reset()
        state_idx = np.argmax(state[:-2])  # Convert state to an index for the Q-table
        total_rewards = 0
        step = 0
...
state_space = env.observation_space.shape[0]  # The state space is the size of the observation
action_space = env.action_space.n
agent = QLearningAgent(action_space, state_space)

# Train the agent
episodes = 1000
max_steps_per_episode = 100  # Set a maximum number of steps per episode to prevent infinite loops
